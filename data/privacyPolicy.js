...
export const PRIVACY_POLICY = {
  fr: {
    ...
    summary: 'Pas de compte, pas de serveur, pas de publicité, pas de profilage. Vos données (Invaders flashés, profil, badges, statistiques) restent sur votre appareil. Seules les données nécessaires au calcul d’itinéraire et à la recherche d’adresse sont transmises à nos prestataires de cartographie.',
    sections: [
      { title: 'Éditeur & contact', body: `InvaderQuest est un projet indépendant édité par son développeur. Pour toute question : ${CONTACT}.` },
      { title: 'Aucun compte, aucun serveur', body: 'L’application ne crée pas de compte et n’a aucun serveur vous concernant. Aucune donnée personnelle n’est collectée sur nos infrastructures. Il n’y a ni publicité, ni traceur publicitaire, ni profilage : rien de ce qui est mesuré ne permet de vous identifier ni de vous suivre d’une application à l’autre.' },
      ...
      { title: 'Services tiers', body: 'Pour certaines fonctions, l’application contacte : Mapbox (recherche d’adresse — reçoit le texte saisi et une position approximative), OpenRouteService (itinéraires — reçoit les points de départ et d’arrivée), GitHub (téléchargement de la base publique des Invaders, sans donnée personnelle), Aptabase (mesure d’usage anonyme et agrégée, détaillée à la section suivante ; hébergement en Europe) et Expo (mises à jour logicielles, données techniques minimales). Ces échanges se limitent au strict nécessaire et ne servent ni à la publicité ni au profilage.' },
      { title: "Mesure d’usage", body: "Pour savoir quelles fonctions servent réellement et lesquelles échouent, l’application envoie des événements anonymes et agrégés à Aptabase (hébergé en Europe). Sont transmis : l’écran affiché, l’usage des fonctions (chasse ou trajet calculé, démarré, terminé, partagé), les réglages choisis (durée, mode de déplacement, objectif), la ville sélectionnée, les erreurs rencontrées, et des indicateurs chiffrés agrégés (nombre d’étapes, distance et durée d’une session, nombre d’Invaders flashés pendant celle-ci). Ne sont jamais transmis : votre position, les adresses que vous saisissez, l’identité des Invaders ou des lieux concernés, votre pseudonyme, votre photo, ni aucun identifiant publicitaire ou d’appareil. Ces mesures ne permettent ni de vous identifier, ni de reconstituer votre parcours." },
      ...
      { title: 'Partage d’image', body: 'La fonction de partage génère une image sur votre appareil et l’ouvre dans la feuille de partage du téléphone. Vous seul décidez de la partager ; l’application ne publie rien en votre nom. Le visuel d’une chasse ou d’un trajet peut inclure une carte du secteur parcouru (fournie par Mapbox / OpenStreetMap).' },
  ...
  en: {
    ...
    summary: 'No account, no server, no ads, no profiling. Your data (flashed Invaders, profile, badges, stats) stays on your device. Only the data needed for routing and address search is sent to our mapping providers.',
    sections: [
      { title: 'Publisher & contact', body: `InvaderQuest is an independent project published by its developer. Any question: ${CONTACT}.` },
      { title: 'No account, no server', body: 'The app has no account and no server about you. No personal data is collected on our infrastructure. There are no ads, no advertising trackers and no profiling: nothing we measure can identify you or follow you across apps.' },
      ...
      { title: 'Third-party services', body: 'For some features the app contacts: Mapbox (address search — receives the typed text and an approximate location), OpenRouteService (routing — receives start and end points), GitHub (download of the public Invaders database, no personal data), Aptabase (anonymous, aggregated usage measurement, detailed in the next section; hosted in Europe) and Expo (software updates, minimal technical data). These exchanges are limited to what is strictly necessary and are not used for advertising or profiling.' },
      { title: "Usage measurement", body: "To learn which features actually get used and which ones fail, the app sends anonymous, aggregated events to Aptabase (hosted in Europe). Sent: the screen being shown, feature usage (hunt or route calculated, started, finished, shared), the settings you chose (duration, travel mode, objective), the selected city, errors encountered, and aggregated figures (number of stops, distance and duration of a session, number of Invaders flashed during it). Never sent: your location, the addresses you type, which Invaders or places are involved, your nickname, your photo, or any advertising or device identifier. This measurement cannot identify you or reconstruct your route." },
      ...
      { title: 'Image sharing', body: 'The share feature generates an image on your device and opens it in the phone’s share sheet. Only you decide to share it; the app posts nothing on your behalf. A hunt or route recap image may include a map of the area you walked (provided by Mapbox / OpenStreetMap).' },
  ...
  es: {
    ...
    summary: 'Sin cuenta, sin servidor, sin publicidad, sin perfilado. Tus datos (Invaders fotografiados, perfil, insignias, estadísticas) permanecen en tu dispositivo. Solo se envían a nuestros proveedores de mapas los datos necesarios para calcular rutas y buscar direcciones.',
    sections: [
      ...
      { title: 'Sin cuenta, sin servidor', body: 'La aplicación no crea cuenta ni tiene ningún servidor sobre ti. No se recopila ningún dato personal en nuestra infraestructura. No hay publicidad, ni rastreadores publicitarios, ni perfilado: nada de lo que medimos permite identificarte ni seguirte entre aplicaciones.' },
      ...
      { title: 'Servicios de terceros', body: 'Para algunas funciones la app contacta con: Mapbox (búsqueda de direcciones — recibe el texto introducido y una ubicación aproximada), OpenRouteService (rutas — recibe los puntos de salida y llegada), GitHub (descarga de la base pública de Invaders, sin datos personales), Aptabase (medición de uso anónima y agregada, detallada en la sección siguiente; alojamiento en Europa) y Expo (actualizaciones de software, datos técnicos mínimos). Estos intercambios se limitan a lo estrictamente necesario y no se usan para publicidad ni perfilado.' },
      { title: "Medición de uso", body: "Para saber qué funciones se usan realmente y cuáles fallan, la aplicación envía eventos anónimos y agregados a Aptabase (alojado en Europa). Se transmiten: la pantalla mostrada, el uso de las funciones (caza o ruta calculada, iniciada, terminada, compartida), los ajustes elegidos (duración, modo de desplazamiento, objetivo), la ciudad seleccionada, los errores encontrados e indicadores numéricos agregados (número de paradas, distancia y duración de una sesión, número de Invaders marcados durante ella). Nunca se transmiten: tu ubicación, las direcciones que escribes, la identidad de los Invaders o lugares implicados, tu apodo, tu foto, ni ningún identificador publicitario o de dispositivo. Estas mediciones no permiten identificarte ni reconstruir tu recorrido." },
      ...
      { title: 'Compartir imagen', body: 'La función de compartir genera una imagen en tu dispositivo y la abre en el menú de compartir del teléfono. Solo tú decides compartirla; la app no publica nada en tu nombre. La imagen de una caza o ruta puede incluir un mapa de la zona recorrida (proporcionado por Mapbox / OpenStreetMap).' },
  ...
  it: {
    ...
    summary: 'Nessun account, nessun server, nessuna pubblicità, nessuna profilazione. I tuoi dati (Invader flashati, profilo, badge, statistiche) restano sul tuo dispositivo. Vengono inviati ai nostri fornitori di mappe solo i dati necessari al calcolo dei percorsi e alla ricerca di indirizzi.',
    sections: [
      ...
      { title: 'Nessun account, nessun server', body: 'L’app non crea account e non ha alcun server che ti riguarda. Nessun dato personale viene raccolto sulla nostra infrastruttura. Non ci sono pubblicità, tracker pubblicitari né profilazione: nulla di ciò che misuriamo permette di identificarti o di seguirti tra le app.' },
      ...
      { title: 'Servizi di terze parti', body: 'Per alcune funzioni l’app contatta: Mapbox (ricerca indirizzi — riceve il testo digitato e una posizione approssimativa), OpenRouteService (percorsi — riceve i punti di partenza e arrivo), GitHub (download del database pubblico degli Invader, senza dati personali), Aptabase (misurazione d’uso anonima e aggregata, descritta nella sezione seguente; hosting in Europa) ed Expo (aggiornamenti software, dati tecnici minimi). Questi scambi sono limitati allo stretto necessario e non sono usati per pubblicità o profilazione.' },
      { title: "Misurazione d’uso", body: "Per sapere quali funzioni vengono davvero usate e quali falliscono, l’app invia eventi anonimi e aggregati ad Aptabase (con hosting in Europa). Vengono trasmessi: la schermata visualizzata, l’uso delle funzioni (caccia o percorso calcolato, avviato, terminato, condiviso), le impostazioni scelte (durata, modalità di spostamento, obiettivo), la città selezionata, gli errori incontrati e indicatori numerici aggregati (numero di tappe, distanza e durata di una sessione, numero di Invader flashati durante la stessa). Non vengono mai trasmessi: la tua posizione, gli indirizzi che digiti, l’identità degli Invader o dei luoghi coinvolti, il tuo soprannome, la tua foto, né alcun identificatore pubblicitario o di dispositivo. Queste misurazioni non permettono di identificarti né di ricostruire il tuo percorso." },
      ...
      { title: 'Condivisione immagine', body: 'La funzione di condivisione genera un’immagine sul tuo dispositivo e la apre nel menu di condivisione del telefono. Solo tu decidi di condividerla; l’app non pubblica nulla a tuo nome. L’immagine di una caccia o di un percorso può includere una mappa della zona percorsa (fornita da Mapbox / OpenStreetMap).' },
