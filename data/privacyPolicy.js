...
export const PRIVACY_POLICY = {
  ...
  en: {
    ...
    sections: [
      ...
      { title: 'Third-party services', body: 'For some features the app contacts: Mapbox (address search — receives the typed text and an approximate location), OpenRouteService (routing — receives start and end points), GitHub (download of the public Invaders database, no personal data), Aptabase (anonymous, aggregated usage analytics — screens visited and app opens; no personal data, no advertising identifier, hosted in Europe) and Expo (software updates, minimal technical data). These exchanges are limited to what is strictly necessary and are not used for advertising or profiling.' },
  ...
  it: {
    ...
    sections: [
      ...
      { title: 'Servizi di terze parti', body: 'Per alcune funzioni l’app contatta: Mapbox (ricerca indirizzi — riceve il testo digitato e una posizione approssimativa), OpenRouteService (percorsi — riceve i punti di partenza e arrivo), GitHub (download del database pubblico degli Invader, senza dati personali), Aptabase (statistiche d’uso anonime e aggregate — schermate visitate e aperture dell’app; nessun dato personale, nessun identificatore pubblicitario, hosting in Europa) ed Expo (aggiornamenti software, dati tecnici minimi). Questi scambi sono limitati allo stretto necessario e non sono usati per pubblicità o profilazione.' },
